...
const Signup = ({ setUser }: SignupProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success("Account created successfully! Now please sign in to confirm your credentials.");
        
        /**
         * Updated Logic: Automatically log the user in.
         * We extract the user info (from data.user or data depending on your API structure)
         * and save it to localStorage so the session persists.
         */
        const newUser = data.user || data;
        
        localStorage.setItem("waymark_user", JSON.stringify(newUser));
                ...
                <div className="space-y-1.5 pt-1">
                  {passwordChecks.map((check) => (
                    <div key={check.label} className="flex items-center gap-2">
                      ...
                      <span className={`text-xs transition-colors ${check.met ? "text-green-500" : "text-muted-foreground/60"}`}>{check.label}</span>
                    </div>
                  ))}
                </div>
              )}
