  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Save to localStorage so App.tsx sees you are logged in
        localStorage.setItem("waymark_user", JSON.stringify(data));
        setUser(data);
        navigate("/dashboard");
      } else {
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Link to="/forgot-password" size="sm" className="text-xs text-primary">Forgot password?</Link>
              </div>
            ...
            <Button type="submit" variant="hero" className="w-full h-12" disabled={isLoading}>
              {isLoading ? <div className="h-5 w-5 border-2 border-t-transparent rounded-full animate-spin" /> : <>Sign In <ArrowRight className="h-4 w-4 ml-1" /></>}
            </Button>
          </form>
